...
extension OnDeviceModel {
    ...
    private static func instructionsBody(tagSection: String) -> String {
        ...
        - 抜き出す主題は、店名・スポット名・施設名・イベント名など“その対象そのものの固有名詞”。
          「店舗概要」「アクセス」「メニュー」「ご予約」「料金」「営業時間」「よくあるご質問」「トップ」「お知らせ」等の
          ページのセクション名・ナビ見出しは主題にしない。タイトルが「セクション名 | 店名」の形なら、店名の側を主題にする。
        ...
        - ページタイトル「店舗概要 | XXXX YYYY店」→「XXXX YYYY店」（「店舗概要」はセクション名なので使わない）
        - ページタイトル「アクセス・地図｜箱根○○美術館」→「箱根○○美術館」
