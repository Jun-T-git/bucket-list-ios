...
で本体ホームバナーと同一のタイミング提案を出す（Small/Medium・タップはアプリ起動のみでディープリンク無し）。
...
| `WidgetExtension/` | ホームウィジェット（`WishesWidget`/`WishesProvider`/`WishesWidgetView`）。`SharedStore.snapshot()`＋`TimingEngine` で提案表示 |
