...
- `LossyArray` / `LossyDocument` は要素ごとにデコード → 1件破損はその1件だけ失う。
  ただし items が**全件デコード失敗**（スキーマ全壊の兆候）なら `.unreadable` 扱い → 空リストで上書きせずバックアップ復旧へ。
- `BucketItem` は手書きの寛容な `Codable` → 新フィールドは既定値フォールバック。

## アプリ ↔ 拡張の共有

3ターゲットとも `com.apple.security.application-groups = group.teratech.BucketList` を
...
ウィジェット（`WidgetExtension`）は `SharedStore.snapshot()` から **読み取り専用** で items を取り、`TimingEngine.suggestion(items:)`
で本体ホームバナーと同一のタイミング提案を出す（ホーム画面 Small/Medium ＋ ロック画面 accessoryRectangular/accessoryInline）。
各面は表示中の wish へ `wishes://item/<id>`（`WishLink`）でディープリンクし、タップで本体 `ContentView.onOpenURL` が該当 wish の
詳細を開く（Small/accessory は `widgetURL`、Medium は行ごとに `Link`）。
アイテム変更後は `WidgetCenter.reloadAllTimelines()` でタイムラインを更新（本体 `AppStore.persistDocument` の保存成功時と、
共有 `ShareViewController.complete()` の完了時）→ ウィジェットがリストに遅れない。
...
| `Models.swift` | モデル＋`AppStore`＋業務ロジック（分類/通知/提案/永続化）。約1800行の中心。タイミング提案は純関数 `TimingEngine`（`[BucketItem]` 入力）に抽出し本体とウィジェットで共有 |
...
| `WidgetExtension/` | ホーム/ロック画面ウィジェット（`WishesWidget`/`WishesProvider`/`WishesWidgetView`）。`SharedStore.snapshot()`＋`TimingEngine` で提案表示。表示中の wish へ `WishLink` でディープリンク |
